...
var Ads = {
	...
	get: function(){
		if(!$.G.controller || !$.G.action || !$('.ad').length) return ;
		var d={};
		d.pages = ('home'===$.G.controller && $.G.action==='index')?'main':$.G.action;
		if($.G.mid) d.mid=$.G.mid;
		if($.G.catenode) d.cid=$.G.catenode;
		$.getJSON($.G.webroot+'cp/ads/get', d, this.build);
	},
	build: function(d){
		if (parseInt(d.code,10) !== 1) return ;
		var a={};
		d = d.body;
		$.each(d,function(i,b){
